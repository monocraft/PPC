...
- **Fit products** scales the longest visible product lane into the available width for an instant portfolio overview
...
- **Fit products** to fit the longest visible product lane into the Product Cards viewport; use **100%** to return to full-size cards
...
Product Cards now uses the same one-click fitting idea as Roadmap. **Fit products** calculates the width of the longest currently visible lane and scales the board down just enough to show it within the available canvas width. Search results are respected, the frozen Wired/Wireless rail remains fixed, and the standard zoom controls can be used immediately afterward.
